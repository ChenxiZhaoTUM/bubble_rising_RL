#!/usr/bin/env python3
"""
SAC training script for the bottom-wall five-segment fast-rising task.

Task:
    Single objective:
        Reach center_y >= target_height as fast as possible.

Important setup:
    - n_seg = 5.
    - C++ module = br_2d_bubble_rising_bottom_python.
    - action_amplitude default = 0.7.
    - mean bottom-wall temperature is still fixed by the C++ zero-mean action
      transformation.
    - A zero-action passive baseline is rolled out before training and is NOT
...
# This package should register BR-v0 to BubbleRisingPositionEnv.
import gym_env_br
...
def rebuild_training_root(training_root: str, clear: bool = True) -> str:
    ...
    if base != "bubble_rising_fast_y15_bottom5":
        raise RuntimeError(
            f"Refuse to clear unexpected directory:\n"
            f"  {training_root}\n"
            f"Expected basename == 'bubble_rising_fast_y15_bottom5'."
        )
...
def get_args():
    ...
    # Mean temperature remains fixed by the C++ action transformation.
    # Increasing amplitude only strengthens the spatial temperature contrast.
    ...
    # Passive baseline.
    # -------------------------------------------------------------------------
    parser.add_argument("--run-baseline", default=True, action="store_true")
    parser.add_argument(
        "--no-run-baseline",
        dest="run_baseline",
        action="store_false",
    )
    parser.add_argument("--baseline-parallel-env", type=int, default=777)
    parser.add_argument("--baseline-entry-time", type=float, default=None)
    parser.add_argument("--baseline-metrics-csv", type=str, default=None)
    parser.add_argument("--baseline-write-output", default=False, action="store_true")
    ...
    parser.add_argument("--start-episodes", type=int, default=2)
    ...
    parser.add_argument("--best-output-steps", type=int, default=225)
...
def make_br_env(
    ...
    return gym.make(
        ...
        baseline_entry_time=baseline_entry_time,
...
# =============================================================================
# Passive baseline rollout.
# =============================================================================
def run_zero_action_baseline(args, training_root: str) -> tuple[str, Optional[float]]:
    """
    Run a passive zero-action episode before training.

    This baseline is not collected by Tianshou and therefore is not inserted into
    the replay buffer.
    """
    print("\n[Baseline] Running passive zero-action baseline outside replay buffer.")

    env = make_br_env(
        parallel_envs=args.baseline_parallel_env,
        training_root=training_root,
        args=args,
        write_output_override=args.baseline_write_output,
    ...
    for _ in range(int(args.max_steps_per_episode)):
        ...
        if entry_time is None and float(info["center_y"]) >= float(args.target_height):
            entry_time = float(info["physical_time"])
    ...
    metrics_csv = os.path.join(
        ...
        f"output_env_{args.baseline_parallel_env}_episode_1",
        "bubble_control_metrics.csv",
    )

    env.close()

    if not os.path.isfile(metrics_csv):
        raise RuntimeError(f"Baseline metrics CSV was not created: {metrics_csv}")

    print(f"[Baseline] metrics_csv = {metrics_csv}")
    print(f"[Baseline] entry_time(center_y >= {args.target_height}) = {entry_time}")
    print(
        "[Baseline] final_center = "
        f"({final_info.get('center_x', None)}, {final_info.get('center_y', None)})"
    )

    return metrics_csv, entry_time
...
def training_sac(args=None):
    ...
    training_root = os.path.join(proj_root, "bubble_rising_fast_y15_bottom5")
    ...
    print(f"[BR] target_height: {args.target_height}")
    print(f"[BR] episode_max_time: {args.episode_max_time}")
    print(f"[BR] action_amplitude: {args.action_amplitude}")
    print(f"[BR] mean_temperature: {args.mean_temperature}")
    print(f"[BR] generate_best_output: {args.generate_best_output}")

    # -------------------------------------------------------------------------
    # Passive baseline outside replay buffer.
    # -------------------------------------------------------------------------
    if args.run_baseline:
        baseline_metrics_csv, baseline_entry_time = run_zero_action_baseline(
            args=args,
            training_root=training_root,
        )
        args.baseline_metrics_csv = baseline_metrics_csv
        args.baseline_entry_time = baseline_entry_time

    else:
        print("[Baseline] Skipped.")
        print(f"[Baseline] baseline_metrics_csv = {args.baseline_metrics_csv}")
        print(f"[Baseline] baseline_entry_time = {args.baseline_entry_time}")

    if args.baseline_metrics_csv:
        print(f"[BR] Using baseline_metrics_csv: {args.baseline_metrics_csv}")
    if args.baseline_entry_time is not None:
        print(f"[BR] Using baseline_entry_time: {args.baseline_entry_time}")
