...
# =============================================================================
# Fixed open-loop actions for control-authority test.
# C++ maps actions to segment temperatures by:
#     T_i = mean_temperature + amplitude * (a_i - mean(a))
# =============================================================================
FIXED_ACTIONS = [
    ("zero_baseline", [0.0, 0.0, 0.0, 0.0]),
    ("up_up_down_down", [1.0, 1.0, -1.0, -1.0]),
    ("down_down_up_up", [-1.0, -1.0, 1.0, 1.0]),
    ("up_down_up_down", [1.0, -1.0, 1.0, -1.0]),
    ("down_up_down_up", [-1.0, 1.0, -1.0, 1.0]),
    ("up_zero_zero_down", [1.0, 0.0, 0.0, -1.0]),
    ("down_zero_zero_up", [-1.0, 0.0, 0.0, 1.0]),
]
...
def metrics_row(
    ...
    return {
        ...
        "temp_0": float(seg_temps[0]),
        "temp_1": float(seg_temps[1]),
        "temp_2": float(seg_temps[2]),
        "temp_3": float(seg_temps[3]),
...
def run_one_fixed_action(
    ...
    # Match the Gym environment reset: start from uniform left-wall temperature.
    sim.set_left_wall_segment_temperatures(
        [mean_temperature] * len(action),
        True,
        mean_temperature,
    )
    ...
    seg_temps = list(sim.get_left_wall_segment_temperatures())

    print(f"[Control] segment temperatures={seg_temps}")
...
# =============================================================================
# Bubble rising heat pybind fixed-action open-loop test.
# =============================================================================
def run_case():
    ...
    parser.add_argument(
        "--skip_zero_baseline",
        action="store_true",
        help="Skip the zero-action baseline case and run only the six nonzero fixed actions.",
    )
